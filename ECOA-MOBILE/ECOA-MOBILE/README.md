...
>
> Aplicativo mobile do ecossistema ECOA. Cidadãos registram ocorrências, acompanham respostas, aprendem boas práticas e fortalecem a zeladoria do próprio território.
...
│   ├── courses.tsx             # Listagem de trilhas ECOA Academy
...
- **Chatbot ECOA**: Assistente contextual baseado em FLAN-T5
...
Crie o arquivo `.env` na raiz de `ECOA-MOBILE/`:

```env
# URL da API ECOA Backend
...
- Backend ECOA rodando (`EXPO_PUBLIC_API_URL` apontando para ele)
...
*ECOA Mobile — Evidências locais, participação cidadã e resposta acompanhável.*

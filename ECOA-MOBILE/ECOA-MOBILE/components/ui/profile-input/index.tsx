...
const inputStyle = tva({
  base: 'h-9 w-full flex-row items-center rounded-md border border-border dark:bg-input/30 bg-transparent shadow-xs overflow-hidden data-[disabled=true]:opacity-50 px-3 gap-2',
});
...
const inputFieldStyle = tva({
  base: 'flex-1 text-foreground text-sm md:text-sm py-1 h-full placeholder:text-muted-foreground web:outline-none ios:leading-[0px] web:cursor-text',
});
...
const Input = React.forwardRef<View, InputProps>(
  function Input({ className, children, isDisabled, isReadOnly, ...props }, ref) {
    ...
    return (
      <InputContext.Provider value={{ inputRef, disabled: isDisabled, readOnly: isReadOnly }}>
        <View
          ref={ref}
          {...props}
...
const InputField = React.forwardRef<TextInput, InputFieldProps>(
  function InputField({ className, editable, readOnly, isReadOnly, isDisabled, ...props }, ref) {

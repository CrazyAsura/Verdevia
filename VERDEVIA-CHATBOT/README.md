# VERDEVIA — Chatbot & AI Domain Microservice

Este repositório contém a infraestrutura e os sub-projetos modulares do domínio de **Chatbot & Inteligência Artificial (AI)** do ecossistema VERDEVIA. Ele é projetado como um microsserviço independente com contingência e suporte a RAG (Retrieval-Augmented Generation).
...
1. **`VERDEVIA-CHATBOT-BACKEND`**: Servidor de backend construído em NestJS utilizando arquitetura hexagonal (Clean Architecture) e federado ao Apollo Gateway como um subgraph GraphQL na porta `3338`. Controla a orquestração de LLMs e serviços RAG.
2. **`VERDEVIA-WEB-CHATBOT`**: Portal web em Next.js que oferece uma interface de chat imersiva e responsiva em tela cheia para suporte, esclarecimento de dúvidas sobre reciclagem e triagem de denúncias ambientais.
3. **`VERDEVIA-AI-GATEWAY`**: Serviço Python FastAPI integrado para processamento local/remoto de IA e detecção de deepfakes, servindo como motor de inferência.
...
*VERDEVIA Chatbot — Triagem inteligente e suporte ambiental automatizado.*

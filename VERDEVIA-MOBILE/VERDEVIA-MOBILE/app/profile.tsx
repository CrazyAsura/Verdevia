...
export default function ProfileScreen() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      setUser({
        id: profileData.id,
        name: profileData.realName || profileData.name || 'Usuário VERDEVIA',
          ...
          <Box className={`w-full mt-4 ${isDark ? 'bg-[#101010] border-[#222]' : 'bg-white border-[#EEE] shadow-sm'} p-5 rounded-[24px] border`}>
            <HStack className="justify-between items-center">
              <VStack className="flex-1 pr-4">
                <Text className="text-[#00FF9C] text-[10px] font-black uppercase tracking-widest">Passe VERDEVIA</Text>

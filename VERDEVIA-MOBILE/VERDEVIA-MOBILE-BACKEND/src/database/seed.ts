...
const PLATFORM_ADMIN_PASSWORD = 'VERDEVIAAdmin2026Seguro';
const CONTRACTOR_PASSWORD = 'VERDEVIAContratante2026';

async function bootstrap() {
  const app = await NestFactory.createApplicationContext(AppModule);
  const dataSource = app.get(DataSource);

  console.log('🚀 Iniciando Mega Seeding VERDEVIA...');
  ...
  const seedCredentials = {
    admin: {
      email: envOrDefault('SEED_ADMIN_EMAIL', 'admin@VERDEVIA.app'),
      password: envOrDefault('SEED_ADMIN_PASSWORD', PLATFORM_ADMIN_PASSWORD),
      realName: envOrDefault('SEED_ADMIN_NAME', 'Matheus Leon'),
    },
    superAdmin: {
      email: envOrDefault('SEED_SUPER_ADMIN_EMAIL', 'superadmin@VERDEVIA.app'),
    ...
    contractor: {
      email: envOrDefault('SEED_CONTRACTOR_EMAIL', 'contractor@VERDEVIA.app'),
    ...
    superContractor: {
      email: envOrDefault('SEED_SUPER_CONTRACTOR_EMAIL', 'supercontractor@VERDEVIA.app'),

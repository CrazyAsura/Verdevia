...
async function bootstrap() {
  ...
  console.log(
    `\x1b[32m[VERDEVIA]\x1b[0m Backend running on: http://localhost:${port}`,
  );
  console.log(
    `\x1b[32m[VERDEVIA]\x1b[0m GraphQL playground: http://localhost:${port}/graphql`,
  );
  if (!isProd) {
    console.log(
      `\x1b[32m[VERDEVIA]\x1b[0m Swagger docs: http://localhost:${port}/api/docs`,
    );
  }
  console.log(
    `\x1b[33m[VERDEVIA]\x1b[0m Environment: ${process.env.NODE_ENV ?? 'development'}`,
  );
  console.log(
    `\x1b[33m[VERDEVIA]\x1b[0m Security: Helmet ✅ | CORS ✅ | Rate Limit ✅ | XSS Guard ✅ | AES-256 ✅`,
  );
}

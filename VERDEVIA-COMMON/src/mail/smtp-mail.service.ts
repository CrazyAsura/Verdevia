...
export class SmtpMailService {
  ...
  async sendPasswordReset(email: string, displayName?: string, portal?: string) {
    ...
    await this.sendMail({
      to: email,
      subject: 'VERDEVIA - Recuperação de senha',
      text: [
        `Olá, ${name}.`,
        '',
        'Recebemos uma solicitação para redefinir sua senha na VERDEVIA.',
        `Acesse: ${resetUrl}`,
        '',
      ...
      html: `
        <div style="font-family:Arial,sans-serif;background:#050505;color:#f8fafc;padding:28px;border-radius:16px">
          <h1 style="margin:0 0 16px;color:#20c997">Recuperação de senha VERDEVIA</h1>
          <p>Olá, <strong>${this.escapeHtml(name)}</strong>.</p>
          <p>Recebemos uma solicitação para redefinir sua senha na plataforma VERDEVIA.</p>
  ...
  private async sendMail(payload: MailPayload) {
    ...
    try {
      ...
      await this.command(socket, `EHLO VERDEVIA.local`, 250);
      ...
      if (!secure) {
        ...
        await this.command(upgraded, `EHLO VERDEVIA.local`, 250);
  ...
  private buildMessage(from: string, payload: MailPayload) {
    const boundary = `VERDEVIA-${Date.now()}`;
    return [
      `From: ${from}`,
      `To: ${payload.to}`,

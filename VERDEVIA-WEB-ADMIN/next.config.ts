...
const nextConfig: NextConfig = {
  ...
  async rewrites() {
    // ADMIN FRONTEND: SSR rewrites run inside the container against the web-gateway.
    // web-gateway (port 3335) federates admin-backend (port 3338) and other subgraphs.
    // BACKEND_INTERNAL_URL=http://web-gateway:3335 in docker-compose (verdevia_admin_net).

  ...
  return [
    {
      ...
      password: envOrDefault('SEED_ADMIN_PASSWORD', 'VERDEVIAAdmin2026Seguro'),
    },
    {
      email: envOrDefault('SEED_SUPER_ADMIN_EMAIL', 'superadmin@verdevia.app'),
      password: envOrDefault('SEED_SUPER_ADMIN_PASSWORD', 'VERDEVIAAdmin2026Seguro'),
    },
    {
      email: envOrDefault('SEED_CONTRACTOR_EMAIL', 'contractor@verdevia.app'),
      password: envOrDefault('SEED_CONTRACTOR_PASSWORD', 'VERDEVIAContratante2026'),
    },
    {
      email: envOrDefault(
        'SEED_SUPER_CONTRACTOR_EMAIL',
        'supercontractor@verdevia.app',
      ),
      password: envOrDefault(
        'SEED_SUPER_CONTRACTOR_PASSWORD',
        'VERDEVIAContratante2026',
      ),
    },
  ];
}
...
async function updatePostgres(credentials: Credential[]) {
  const client = new Client({
    ...
    database: process.env.DB_NAME || 'VERDEVIA',

# PowerShell script to split VERDEVIA-WEB into MAIN, ADMIN and CHATBOT frontends without copying node_modules

$ROOT = "c:\Users\Matheus\Documents\Leon\mobile\projeto-2\Verdevia"
$SRC_WEB = Join-Path $ROOT "VERDEVIA-WEB"
$SRC_NODE_MODULES = Join-Path $SRC_WEB "node_modules"
...
function Copy-FilteredProject($src, $dest) {
  Ensure-CleanDir $dest
  
  # Copy files and folders from src to dest excluding node_modules and .next
  Get-ChildItem -Path $src | Where-Object { $_.Name -ne "node_modules" -and $_.Name -ne ".next" } | ForEach-Object {
    Copy-Item -Path $_.FullName -Destination $dest -Recurse -Force | Out-Null
  }
  
  # Link node_modules using Junction
  $destNodeModules = Join-Path $dest "node_modules"
  New-Item -ItemType Junction -Path $destNodeModules -Value $SRC_NODE_MODULES | Out-Null
}

Write-Host "Splitting Next.js Web Frontend into 3 modules cleanly..." -ForegroundColor Cyan

# ── 1. Create and Copy to WEB-MAIN ────────────────────────────────────────────
Write-Host "Copying to VERDEVIA-WEB-MAIN..." -ForegroundColor Yellow
Copy-FilteredProject $SRC_WEB $MAIN_DEST
...
Write-Host "Copying to VERDEVIA-WEB-ADMIN..." -ForegroundColor Yellow
Copy-FilteredProject $SRC_WEB $ADMIN_DEST
...
Write-Host "Copying to VERDEVIA-WEB-CHATBOT..." -ForegroundColor Yellow
Copy-FilteredProject $SRC_WEB $CHATBOT_DEST
...
Write-Host "Frontend split completed cleanly and successfully!" -ForegroundColor Green
